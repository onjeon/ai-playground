...
additional_tests = [
    ("diskarte-pinoy", "Gaano Ka Ka-Madiskarte?", "Sukatin ang iyong diskarte level"),
    ("sabong-culture", "Anong Sabongero Ka?", "Alamin kung paano ka sa sabong"),
    ("videoke-song", "Anong Videoke Song Ka?", "Alamin kung anong kanta ang personality mo"),
    ("tsinelas-warrior", "Gaano Ka Ka-Tsinelas?", "Alamin ang tsinelas personality mo"),
    ("filipino-time", "Gaano Ka Sa Filipino Time?", "Sukatin ang punctuality mo"),
    ("utang-na-loob", "Gaano Ka Sa Utang Na Loob?", "Alamin ang gratitude culture mo"),
    ("pakikisama", "Gaano Ka Makipakisama?", "Sukatin ang social harmony skills mo"),
    ("hiya-culture", "Gaano Ka Mahiyain?", "Alamin ang hiya level mo"),
    ("lambing-level", "Gaano Ka Ka-Malambing?", "Sukatin ang sweet level mo"),
    ("diskarte-sa-buhay", "Anong Survival Style Mo?", "Alamin kung paano ka magdiskarte"),
]

for test_id, title, description in additional_tests:
    dir_path = f"src/tests-fil/{test_id}"
    os.makedirs(dir_path, exist_ok=True)
    
    content = f"""export const {test_id.replace('-', '')}Data = {{
  id: '{test_id}',
  title: '{title}',
...
"""
    ...
    with open(os.path.join(dir_path, "data.ts"), 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Created: {test_id}")

print(f"\n✅ Created {len(additional_tests)} more Filipino tests!")
