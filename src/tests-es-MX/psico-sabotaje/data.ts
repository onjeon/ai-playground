// Test de Autosabotaje - Test Psicológico México
export const questions = [
  {
    id: 1,
    text: '¿Cómo te describes respecto a autosabotaje?',
    options: [
      { text: 'No te autosaboteas', type: 'bajo' },
      { text: 'A veces te pones obstáculos', type: 'ocasional' },
      { text: 'Frecuentemente te saboteas', type: 'frecuente' },
      { text: 'Constantemente arruinas tus logros', type: 'constante' },
    ],
  },
  {
    id: 2,
    text: '¿Con qué frecuencia experimentas esto?',
    options: [
      { text: 'Casi nunca', type: 'bajo' },
      { text: 'A veces', type: 'ocasional' },
      { text: 'Frecuentemente', type: 'frecuente' },
      { text: 'Casi siempre', type: 'constante' },
    ],
  },
  {
    id: 3,
    text: '¿Cómo afecta esto tu vida diaria?',
    options: [
      { text: 'No me afecta', type: 'bajo' },
      { text: 'Me afecta un poco', type: 'ocasional' },
      { text: 'Me afecta bastante', type: 'frecuente' },
      { text: 'Me afecta mucho', type: 'constante' },
    ],
  },
  {
    id: 4,
    text: '¿Cómo te sientes al respecto?',
    options: [
      { text: 'Muy bien, en paz', type: 'bajo' },
      { text: 'Bien, con algunas dudas', type: 'ocasional' },
      { text: 'Incómodo/a frecuentemente', type: 'frecuente' },
      { text: 'Mal, me genera mucho malestar', type: 'constante' },
    ],
  },
  {
    id: 5,
    text: '¿Cómo impacta tus relaciones?',
    options: [
      { text: 'Positivamente', type: 'bajo' },
      { text: 'Neutral', type: 'ocasional' },
      { text: 'A veces negativamente', type: 'frecuente' },
      { text: 'Muy negativamente', type: 'constante' },
    ],
  },
  {
    id: 6,
    text: '¿Has intentado trabajar en esto?',
    options: [
      { text: 'No es necesario, estoy bien', type: 'bajo' },
      { text: 'Un poco, cuando puedo', type: 'ocasional' },
      { text: 'Sí, pero me cuesta', type: 'frecuente' },
      { text: 'Sí, necesito ayuda', type: 'constante' },
    ],
  },
  {
    id: 7,
    text: '¿Cómo se compara con hace un año?',
    options: [
      { text: 'Igual de bien o mejor', type: 'bajo' },
      { text: 'Más o menos igual', type: 'ocasional' },
      { text: 'Ha empeorado un poco', type: 'frecuente' },
      { text: 'Ha empeorado mucho', type: 'constante' },
    ],
  },
  {
    id: 8,
    text: '¿Cuánta energía te consume?',
    options: [
      { text: 'Nada', type: 'bajo' },
      { text: 'Un poco', type: 'ocasional' },
      { text: 'Bastante', type: 'frecuente' },
      { text: 'Demasiada', type: 'constante' },
    ],
  },
  {
    id: 9,
    text: '¿Cómo te describirían otros al respecto?',
    options: [
      { text: 'Muy bien equilibrado/a', type: 'bajo' },
      { text: 'Normal', type: 'ocasional' },
      { text: 'Que tengo un problema con esto', type: 'frecuente' },
      { text: 'Que necesito ayuda urgente', type: 'constante' },
    ],
  },
  {
    id: 10,
    text: '¿Interfiere con tu trabajo o estudios?',
    options: [
      { text: 'No, nunca', type: 'bajo' },
      { text: 'Rara vez', type: 'ocasional' },
      { text: 'A veces', type: 'frecuente' },
      { text: 'Frecuentemente', type: 'constante' },
    ],
  },
  {
    id: 11,
    text: '¿Te genera ansiedad o estrés?',
    options: [
      { text: 'No', type: 'bajo' },
      { text: 'Un poco', type: 'ocasional' },
      { text: 'Bastante', type: 'frecuente' },
      { text: 'Mucho', type: 'constante' },
    ],
  },
  {
    id: 12,
    text: '¿Cómo calificarías tu situación general?',
    options: [
      { text: 'Excelente', type: 'bajo' },
      { text: 'Buena', type: 'ocasional' },
      { text: 'Regular', type: 'frecuente' },
      { text: 'Mala', type: 'constante' },
    ],
  },
];

export const results: Record<string, {
  type: string;
  title: string;
  emoji: string;
  percentage: string;
  description: string;
  characteristics: string[];
  advice: string;
}> = {
  bajo: {
    type: 'bajo',
    title: 'No Te Saboteas',
    emoji: '💚🚀',
    percentage: 'Nivel saludable',
    description: 'No te autosaboteas. Tu manejo de este aspecto es muy saludable.',
    characteristics: ['Equilibrado/a', 'Funcional', 'Saludable', 'Resiliente', 'Estable'],
    advice: 'Sigue así. Tu manejo de este aspecto de tu vida es admirable.',
  },
  ocasional: {
    type: 'ocasional',
    title: 'Sabotaje Ocasional',
    emoji: '💛⚠️',
    percentage: 'Nivel moderado',
    description: 'A veces te pones obstáculos. Tienes espacio para mejorar pero estás bien.',
    characteristics: ['En general bien', 'Algunos desafíos', 'Funcional', 'Consciente', 'En proceso'],
    advice: 'Vas bien. Considera trabajar un poco más en este aspecto para mejorar tu bienestar.',
  },
  frecuente: {
    type: 'frecuente',
    title: 'Sabotaje Frecuente',
    emoji: '🟠🔄',
    percentage: 'Nivel alto - Atención',
    description: 'Frecuentemente te saboteas. Esto puede estar afectando tu calidad de vida.',
    characteristics: ['Necesita atención', 'Afecta bienestar', 'Patrones problemáticos', 'Espacio para crecer', 'Requiere trabajo'],
    advice: 'Es importante que trabajes en este aspecto. Considera buscar apoyo profesional.',
  },
  constante: {
    type: 'constante',
    title: 'Sabotaje Constante',
    emoji: '🔴💣',
    percentage: 'Nivel severo - Busca ayuda',
    description: 'Constantemente arruinas tus logros. Este nivel puede estar causando sufrimiento significativo.',
    characteristics: ['Necesita ayuda', 'Afecta seriamente', 'Urgente atención', 'Impacto severo', 'Requiere apoyo'],
    advice: 'Te recomendamos buscar ayuda profesional. No tienes que enfrentar esto solo/a.',
  },
};

export function calculateResult(answers: number[]): typeof results[keyof typeof results] {
  const scores: Record<string, number> = { bajo: 0, ocasional: 0, frecuente: 0, constante: 0 };

  answers.forEach((answerIndex, questionIndex) => {
    const question = questions[questionIndex];
    if (question && question.options[answerIndex]) {
      const type = question.options[answerIndex].type;
      scores[type]++;
    }
  });

  const resultType = Object.entries(scores).reduce((a, b) => (b[1] > a[1] ? b : a))[0];
  return results[resultType];
}
